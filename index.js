...
const root= {
  ...
  losersPerRound: async () => {
    // const bets= await Bet.find();
    // let thisMonth=bets.filter(item=>item.createdAt.getMonth()+1 === new Date().getMonth()+1 && item.win === true)

    // let tt=0
    //  thisMonth.map(item=>{
    //   tt=tt+item.amount+item.tax;
    // })
    // console.log(tt)
    const bets = await Bet.aggregate([
      { $match: { win: true,  } },
...
// }
  ...
  //       ...transaction?._doc,
  //       _id: transaction?.id,
  //       user: singleUser.bind(this, transaction?._doc?.user),
